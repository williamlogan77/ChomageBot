"""Add a nullable ``patch_version`` column to ``match_stats``.

Match-V5 responses include ``info.gameVersion`` (e.g. "14.18.612.4234").
...
What this script does, in one transaction:
  ...
  2. Detect whether ``patch_version`` already exists on match_stats. If
     so, exit 0 (idempotent).
  3. ALTER TABLE match_stats ADD COLUMN patch_version TEXT.
  4. Verify the row count is unchanged.

What it does NOT do:
  * Backfill ``patch_version`` for existing rows. That requires a fresh
    Match-V5 fetch per row and is a separate (expensive) operation.
...
Usage:
    python scripts/migrate_add_patch_version.py path/to/database.sqlite
    python scripts/migrate_add_patch_version.py --dry-run path/to/database.sqlite
...
def migrate(db_path: Path, dry_run: bool = False) -> int:
    ...
    try:
        ...
        if "patch_version" in columns:
            print("[migrate] patch_version column already present — nothing to do")
            return 0

        if dry_run:
            print("[migrate] DRY RUN — would ADD COLUMN patch_version TEXT to match_stats")
        ...
        print("[migrate] adding patch_version column…")
        con.execute("BEGIN")
        con.execute("ALTER TABLE match_stats ADD COLUMN patch_version TEXT")
        con.commit()
        ...
        if after != before:
            ...
            return 3
        if "patch_version" not in new_columns:
            print(f"[migrate] ERROR: patch_version not added; backup at {backup_path}")
            return 4

        print("[migrate] done.")
        print("[migrate] Existing rows have NULL patch_version. New inserts via the")
        print("[migrate] backfill cog will populate it from match['info']['gameVersion'].")
        return 0
    finally:
        con.close()
